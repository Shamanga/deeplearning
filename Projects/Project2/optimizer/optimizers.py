...
class AdagradOptimizer(optimizer):
    ...
    def update(self, model):
        ...
        for i, p in enumerate(model.parameters()):
            ...
            p.grad = self.learning_rate * p.grad /(self.delta + torch.sqrt(self.r[i]))
...
class AdamOptimizer(optimizer):
    ...
    def update(self, model):
        ...
        for i, p in enumerate(model.parameters()):
            ...
            m1_hat = self.m1[i] /(1-torch.pow(self.beta1, torch.FloatTensor([self.iteration + 1])))
            m2_hat = self.m2[i] /(1-torch.pow(self.beta2, torch.FloatTensor([self.iteration + 1])))
